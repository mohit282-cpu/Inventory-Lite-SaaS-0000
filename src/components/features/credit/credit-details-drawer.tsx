  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Updated payment details to Rs. ${formatMoney(amount)}.`,
      })
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={(open) => { if (!open) onClose() }}>
        <DialogContent className="max-w-xl w-[95vw] sm:w-full border-slate-200 bg-white text-slate-900 shadow-xl max-h-[90vh] flex flex-col p-0 overflow-hidden">
          <DialogHeader className="p-5 pb-3 border-b border-slate-100 shrink-0">
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <DialogTitle className="text-lg sm:text-xl font-bold text-slate-900">{item.customerName}</DialogTitle>
          ...
          <div className="flex-1 overflow-y-auto min-h-0 p-5 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="p-3 rounded-xl bg-emerald-50/40 border border-emerald-200 text-center">
              ...
              <div className={`p-3 rounded-xl border text-center transition-colors ${
                hasDue ? 'bg-amber-50/60 border-amber-200' : 'bg-slate-50 border-slate-200'
              }`}>
            ...
            {hasDue && (
              <div className="flex items-center justify-between p-3 rounded-xl bg-indigo-50/60 border border-indigo-100">
                <div className="text-xs font-bold text-indigo-900">
                  Outstanding Balance: <span className="font-mono text-indigo-700">Rs. {formatMoney(item.dueAmount)}</span>
                </div>
                <Button
