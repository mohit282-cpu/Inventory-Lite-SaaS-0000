"use client"

import { AuditOverviewKPIs } from '@/services/audit-center.service'
import { formatCurrency } from '@/lib/utils'
import {
  ShoppingCart,
  Receipt,
  ShoppingBag,
  RotateCcw,
  Percent,
  Wallet,
  Building2,
  Boxes,
  TrendingUp,
  CreditCard,
  DollarSign,
  ShieldCheck,
  ArrowRight,
  TrendingDown,
} from 'lucide-react'

interface AuditOverviewTabProps {
  kpis: AuditOverviewKPIs | null
  loading: boolean
  onSelectTab: (tabId: string) => void
}

interface KpiCardData {
  id: string
  label: string
  value: string
  subtext: string
  icon: any
  color: string
  isNegative?: boolean
  tooltip: string
  action: () => void
}

export function AuditOverviewTab({ kpis, loading, onSelectTab }: AuditOverviewTabProps) {
  if (loading || !kpis) {
    return (
      <div className="space-y-6 animate-pulse">
        <div className="h-16 bg-slate-200/80 rounded-xl" />
        {[1, 2, 3, 4].map((sectionIndex) => (
          <div key={sectionIndex} className="space-y-3">
            <div className="h-4 w-36 bg-slate-200/80 rounded" />
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="h-32 bg-slate-200/80 rounded-xl" />
              ))}
            </div>
          </div>
        ))}
      </div>
    )
  }

  // Section A: Sales & Revenue
  const salesAndRevenueCards: KpiCardData[] = [
    {
      id: 'total-sales',
      label: 'Total Sales',
      value: formatCurrency(kpis.totalSales),
      subtext: `${kpis.totalSalesCount} total sales recorded`,
      icon: ShoppingCart,
      color: 'text-indigo-600 bg-indigo-50 border-indigo-100',
      tooltip: 'Gross value of all completed sales in selected period.',
      action: () => onSelectTab('sales'),
    },
    {
      id: 'total-bills',
      label: 'Total Bills Issued',
      value: kpis.totalBills.toString(),
      subtext: 'Customer invoice documents',
      icon: Receipt,
      color: 'text-blue-600 bg-blue-50 border-blue-100',
      tooltip: 'Total count of customer sales invoices generated.',
      action: () => onSelectTab('sales'),
    },
    {
      id: 'gross-profit',
      label: 'Gross Profit',
      value: formatCurrency(kpis.grossProfit),
      subtext: 'Net Sales minus COGS',
      icon: TrendingUp,
      color: kpis.grossProfit < 0 ? 'text-red-700 bg-red-50 border-red-200' : 'text-emerald-700 bg-emerald-50 border-emerald-200',
      isNegative: kpis.grossProfit < 0,
      tooltip: 'Gross Profit = Net Sales − COGS.',
      action: () => onSelectTab('profitability'),
    },
    {
      id: 'net-profit',
      label: 'Net Profit',
      value: formatCurrency(kpis.netProfit),
      subtext: 'Gross Profit minus Expenses',
      icon: kpis.netProfit < 0 ? TrendingDown : TrendingUp,
      color: kpis.netProfit < 0 ? 'text-red-700 bg-red-50 border-red-200' : 'text-indigo-700 bg-indigo-50 border-indigo-200',
      isNegative: kpis.netProfit < 0,
      tooltip: 'Net Profit = Gross Profit − Operating Expenses.',
      action: () => onSelectTab('profitability'),
    },
  ]

  // Section B: Purchases & Inventory
  const purchasesAndInventoryCards: KpiCardData[] = [
    {
      id: 'total-purchases',
      label: 'Total Purchases',
      value: formatCurrency(kpis.totalPurchases),
      subtext: `${kpis.totalPurchaseCount} supplier bills recorded`,
      icon: ShoppingBag,
      color: 'text-purple-600 bg-purple-50 border-purple-100',
      tooltip: 'Total cost of goods & stock purchased from suppliers.',
      action: () => onSelectTab('purchases'),
    },
    {
      id: 'cogs',
      label: 'Cost of Goods Sold (COGS)',
      value: formatCurrency(kpis.cogs),
      subtext: 'Direct cost of items sold',
      icon: DollarSign,
      color: 'text-slate-700 bg-slate-100 border-slate-200',
      tooltip: 'COGS = Sum of (unit cost × quantity sold).',
      action: () => onSelectTab('profitability'),
    },
    {
      id: 'stock-value',
      label: 'Stock Asset Value',
      value: formatCurrency(kpis.stockValue),
      subtext: 'Inventory asset at cost price',
      icon: Boxes,
      color: 'text-cyan-600 bg-cyan-50 border-cyan-100',
      tooltip: 'Total cost value of current on-hand inventory.',
      action: () => onSelectTab('inventory'),
    },
    {
      id: 'purchase-returns',
      label: 'Purchase Returns',
      value: formatCurrency(kpis.purchaseReturns),
      subtext: 'Supplier debit notes & returns',
      icon: RotateCcw,
      color: 'text-amber-600 bg-amber-50 border-amber-100',
      tooltip: 'Total stock returned to suppliers.',
      action: () => onSelectTab('returns'),
    },
  ]

  // Section C: Tax & Compliance
  const taxAndComplianceCards: KpiCardData[] = [
    {
      id: 'output-vat',
      label: 'Output VAT (Collected)',
      value: formatCurrency(kpis.outputVat),
      subtext: '13% tax collected from sales',
      icon: Percent,
      color: 'text-emerald-600 bg-emerald-50 border-emerald-100',
      tooltip: 'Output VAT collected on customer sales.',
      action: () => onSelectTab('vat'),
    },
    {
      id: 'input-vat',
      label: 'Input VAT (Claimable)',
      value: formatCurrency(kpis.inputVat),
      subtext: '13% tax paid on purchases',
      icon: Percent,
      color: 'text-teal-600 bg-teal-50 border-teal-100',
      tooltip: 'Input VAT eligible to credit against Output VAT.',
      action: () => onSelectTab('vat'),
    },
    {
      id: 'net-vat',
      label: 'Net VAT Position',
      value: formatCurrency(kpis.netVatPosition),
      subtext: 'Output VAT minus Input VAT',
      icon: Wallet,
      color: 'text-sky-600 bg-sky-50 border-sky-100',
      tooltip: 'Net tax payable to tax office for the period.',
      action: () => onSelectTab('vat'),
    },
  ]

  // Section D: Receivables & Payables
  const receivablesAndPayablesCards: KpiCardData[] = [
    {
      id: 'customer-credit',
      label: 'Outstanding Customer Credit',
      value: formatCurrency(kpis.outstandingCustomerCredit),
      subtext: 'Total Udhar due from customers',
      icon: Building2,
      color: 'text-orange-600 bg-orange-50 border-orange-100',
      tooltip: 'Total unpaid balances owed by customers.',
      action: () => onSelectTab('customers'),
    },
    {
      id: 'supplier-payables',
      label: 'Supplier Payables',
      value: formatCurrency(kpis.supplierPayables),
      subtext: 'Outstanding supplier bills',
      icon: CreditCard,
      color: 'text-red-600 bg-red-50 border-red-100',
      tooltip: 'Total pending amounts owed to suppliers.',
      action: () => onSelectTab('suppliers'),
    },
    {
      id: 'sales-returns',
      label: 'Sales Returns',
      value: formatCurrency(kpis.salesReturns),
      subtext: 'Customer credit notes issued',
      icon: RotateCcw,
      color: 'text-rose-600 bg-rose-50 border-rose-100',
      tooltip: 'Total customer refunds & credit notes.',
      action: () => onSelectTab('returns'),
    },
  ]

  const sections = [
    { title: 'SALES & REVENUE', cards: salesAndRevenueCards },
    { title: 'PURCHASES & INVENTORY', cards: purchasesAndInventoryCards },
    { title: 'TAX & COMPLIANCE', cards: taxAndComplianceCards },
    { title: 'RECEIVABLES & PAYABLES', cards: receivablesAndPayablesCards },
  ]

  const renderCard = (card: KpiCardData) => {
    const Icon = card.icon
    return (
      <div
        key={card.id}
        tabIndex={0}
        role="button"
        aria-label={`View details for ${card.label}`}
        onClick={card.action}
        onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); card.action() } }}
        className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs hover:shadow-md hover:border-indigo-300 transition-all cursor-pointer group flex flex-col justify-between h-full focus:outline-none focus:ring-2 focus:ring-indigo-500"
      >
        <div className="flex items-start justify-between gap-2">
          <span className="text-xs font-bold text-slate-700 group-hover:text-indigo-600 transition-colors leading-snug">
            {card.label}
          </span>
          <div className={`p-2 rounded-lg border shrink-0 ${card.color}`}>
            <Icon className="h-4 w-4" />
          </div>
        </div>

        <div className="my-2">
          <div className={`text-lg font-black tracking-tight ${card.isNegative ? 'text-red-600' : 'text-slate-900'}`}>
            {card.value}
          </div>
          <div className="text-[11px] text-slate-500 font-medium mt-0.5">{card.subtext}</div>
        </div>

        <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs text-slate-400 group-hover:text-indigo-600 font-semibold transition-colors mt-auto">
          <span>View Details</span>
          <ArrowRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-0.5" />
        </div>
      </div>
    )
  }

  return (
    <div className="space-y-6">
      {/* Light Audit Overview Header */}
      <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 flex flex-wrap items-center justify-between gap-3 shadow-2xs">
        <div>
          <h2 className="text-base font-extrabold text-slate-900 flex items-center gap-2">
            Audit Overview
          </h2>
          <p className="text-xs text-slate-500 mt-0.5">
            Financial performance and compliance summary for the selected period.
          </p>
        </div>
        <div className="flex items-center gap-1.5 text-[11px] font-bold text-indigo-900 bg-indigo-50 px-2.5 py-1 rounded-lg border border-indigo-200">
          <ShieldCheck className="h-3.5 w-3.5 text-indigo-600" />
          <span>Tenant Isolation Enforced</span>
        </div>
      </div>

      {/* 4 Categorized KPI Sections */}
      {sections.map((sec) => (
        <div key={sec.title} className="space-y-3">
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500 border-b border-slate-200 pb-1">
            {sec.title}
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {sec.cards.map(renderCard)}
          </div>
        </div>
      ))}
    </div>
  )
}

