  ...
  return (
    <section id="features" className="py-10 sm:py-16 lg:py-20 bg-white text-slate-900 border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-7 sm:space-y-10">
        ...
        {/* Feature Grid - Compact mobile layout */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 sm:gap-3 lg:gap-4 pt-1">
          {featureList.map((feat: string, idx: number) => (
            <div
              key={idx}
              className="flex items-center gap-2.5 sm:gap-3 p-3 sm:p-3.5 rounded-xl bg-slate-50 border border-slate-200 shadow-xs hover:border-indigo-300 transition-colors"
            >
              <CheckCircle2 className="h-4 w-4 sm:h-5 sm:w-5 text-emerald-600 shrink-0" />
              <span className="text-sm sm:text-sm lg:text-base font-bold text-slate-900 leading-snug">
                {feat}
              </span>
            </div>
          ))}
