...
export default function InvoiceDetailPage({ params }: InvoiceDetailPageProps) {
  ...
  if (error || !details) {
    return (
      <div className="flex flex-col items-center justify-center py-16 space-y-4 text-center">
        <div className="rounded-full bg-red-950/50 p-4 text-red-400 border border-red-800">
          <FileText className="h-8 w-8" />
        </div>
        <h2 className="text-xl font-bold text-white">Invoice Not Found</h2>
        <p className="text-slate-400 max-w-md">{error || 'The requested invoice could not be located.'}</p>
        <Button onClick={() => router.push('/app/invoices')} className="bg-slate-800 hover:bg-slate-700 text-white">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900 border border-slate-800 rounded-xl p-4 print:hidden">
          ...
          <div className="flex items-center bg-slate-950 p-1 rounded-lg border border-slate-800 text-xs">
            <button
              ...
              className={`px-3 py-1.5 rounded-md font-medium transition-all ${
                format === 'A4' ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              A4 Tax Invoice
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-md font-medium transition-all ${
                format === 'THERMAL' ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
      ...
      <div className="flex justify-center">
        {format === 'A4' ? (
          ...
          <div className="w-full max-w-4xl bg-white text-slate-900 rounded-xl shadow-2xl p-8 print:p-0 print:shadow-none print:max-w-none print:w-full print:rounded-none">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start border-b border-slate-200 pb-6 mb-6">
              <div>
                ...
                <div className="flex items-center gap-4 mt-2 font-mono text-xs font-semibold text-slate-700 bg-slate-100 px-3 py-1.5 rounded inline-flex border border-slate-200">
              ...
              <div className="mt-4 sm:mt-0 text-left sm:text-right">
                <div className="inline-block bg-slate-900 text-white px-4 py-1.5 rounded font-black text-sm uppercase tracking-wider mb-2">
                  TAX INVOICE
                </div>
                ...
                <p className="text-xs text-slate-500">
                  Payment Method: <span className="font-semibold uppercase">{sale.paymentMethod}</span>
                </p>
              </div>
            </div>
            ...
            <div className="bg-slate-50 rounded-lg border border-slate-200 p-4 mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <p className="text-xs font-bold uppercase text-slate-400 tracking-wider mb-1">Billed To (Customer)</p>
                <p className="font-bold text-slate-900 text-base">{customer ? customer.name : 'Walk-in Customer'}</p>
                ...
                {customer?.panNumber && <p className="text-xs font-mono text-slate-600 mt-1">PAN: {customer.panNumber}</p>}
              </div>

              <div className="md:text-right">
                <p className="text-xs font-bold uppercase text-slate-400 tracking-wider mb-1">Invoice Overview</p>
                <p className="text-sm text-slate-700">
                  Status: <span className="font-bold uppercase text-emerald-700">{sale.status}</span>
                </p>
                <p className="text-sm text-slate-700">
                  Sale Ref: <span className="font-mono">{sale.saleNumber || sale.$id.slice(0, 8)}</span>
                </p>
              </div>
            </div>
                ...
                <tbody className="divide-y divide-slate-200">
                  {saleItems.map((item, idx) => (
                    <tr key={item.$id || idx} className="hover:bg-slate-50">
                      ...
                      <td className="py-3 px-3 font-medium text-slate-900">{item.productNameSnapshot}</td>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start border-t border-slate-200 pt-6">
              <div className="max-w-xs text-xs text-slate-500 space-y-1 mb-4 sm:mb-0">
                ...
                <p className="mt-4 italic font-semibold text-slate-700">Thank you for your business!</p>
              </div>

              <div className="w-full sm:w-72 bg-slate-50 rounded-lg p-4 border border-slate-200 space-y-2">
                <div className="flex justify-between text-sm text-slate-600">
                  <span>Subtotal:</span>
                  <span className="font-mono">Rs. {sale.subtotal.toFixed(2)}</span>
                </div>
                {sale.discount > 0 && (
                  <div className="flex justify-between text-sm text-emerald-700">
                    <span>Discount:</span>
                    <span className="font-mono">- Rs. {sale.discount.toFixed(2)}</span>
                  </div>
                )}
                <div className="flex justify-between text-sm text-slate-600">
                  <span>VAT / Tax (13%):</span>
                  <span className="font-mono">Rs. {sale.tax.toFixed(2)}</span>
                </div>
                <div className="border-t border-slate-300 pt-2 flex justify-between text-base font-black text-slate-900">
                  <span>Grand Total:</span>
                  <span className="font-mono">Rs. {sale.total.toFixed(2)}</span>
                </div>
                <div className="border-t border-slate-200 pt-2 flex justify-between text-xs text-slate-700">
                  <span>Paid Amount:</span>
                  <span className="font-mono font-semibold">Rs. {sale.paidAmount.toFixed(2)}</span>
                </div>
          ...
          <div className="w-[80mm] max-w-full bg-white text-slate-900 shadow-2xl p-4 font-mono text-xs rounded-lg border border-slate-300 print:p-0 print:shadow-none print:border-none print:w-[80mm]">
