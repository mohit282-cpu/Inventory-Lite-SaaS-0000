...
export function validateAppwriteConfig(throwOnError: boolean = true): boolean {
  ...
  if (!config.isConfigured) {
    ...
    const actionGuide = isDev
      ...
      : 'Please set NEXT_PUBLIC_APPWRITE_PROJECT_ID in your hosting provider dashboard.'
    const errorMsg = `Configuration Error: NEXT_PUBLIC_APPWRITE_PROJECT_ID is not configured in ${envSource}. ${actionGuide}`
    
    if (typeof window !== 'undefined') {
      console.warn(`[Appwrite Config] ${errorMsg}`)
    } else {
